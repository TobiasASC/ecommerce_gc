...
<div class="container-fluid px-0 overflow-hidden">
    <div class="row g-0"> 
        ...
        <main class="col-12 col-md-9 col-xl-10 p-3 p-md-4" style="min-width: 0;">
            ...
            <div class="card shadow-sm border-1 rounded-4 busqueda">
                <div class="card-body p-4">
                    
                    <div class="d-flex flex-column flex-xl-row justify-content-between align-items-xl-center mb-3 gap-3">
                        <div class="d-flex align-items-center gap-2">
                            <h3 class="card-title m-0 titulo fw-bold">Mis Pedidos</h3>
                            <span class="pill-counter">{{ $pedidos->total() }} pedidos</span>
                        </div>
                        
                        <form action="{{ route('cliente.pedidos') }}" method="GET" class="d-flex flex-column flex-sm-row align-items-sm-center gap-4" id="formBusqueda">
                            
                            <div class="input-group shadow-sm subtitulo" style="max-width: 350px;">
                                <span class="input-group-text icono-busqueda border-end-0"><i class="fa-solid fa-magnifying-glass text-muted"></i></span>
                        ...
                        <div class="table-responsive panel-card subtitulo" style="max-height: 500px; overflow-y: auto; overflow-x: auto; border-radius: 8px;">
                            <table class="table panel-table align-middle mb-0 w-100">
                                ...
                                @forelse($pedidos as $pedido)
                                    <tr>
                                        <td class="text-center text-secondary fw-bold">{{ $pedido->codigo_pedido }}</td>
                                        <td class="text-center text-secondary">{{ \Carbon\Carbon::parse($pedido->fecha_venta ?? $pedido->created_at)->format('d/m/Y H:i') }}</td>
                                        <td class="text-center">
                                            @switch(strtolower($pedido->estado))
                                                @case('confirmado')
                                                    <span class="badge bg-success rounded-pill px-3 py-2 fs-6 fw-normal shadow-sm" style="width: 130px; display: inline-block;">
                                                        <i class="fa-solid fa-circle-check me-1"></i> Confirmado
                                                    </span>
                                                @break

                                                @case('pendiente')
                                                    <span class="badge bg-warning text-dark rounded-pill px-3 py-2 fs-6 fw-normal shadow-sm" style="width: 130px; display: inline-block;">
                                                        <i class="fa-solid fa-clock me-1"></i> Pendiente
                                                    </span>
                                                @break

                                                @default
                                                    <span class="badge bg-secondary rounded-pill px-3 py-2 fs-6 fw-normal shadow-sm">
                                        ...
                                        <td class="text-center text-nowrap">
                                            <a href="{{ route('cliente.pedidos.detalle', $pedido->id) }}" class="btn btn-sm boton-detalle -info shadow-sm" title="Ver detalle">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="4" class="text-center py-5 text-muted">
                                            ...
                                            <p class="mb-0">Por el momento no se encontraron pedidos.</p>
                                        </td>
                                    </tr>
                                @endforelse
                                </tbody>
